...
code = code.replace(
  "check('dev_pc_kitty', T(4.5), T(6.3), 55, '🌸 [E] Dev Station (Hello Kitty)');",
  "check('dev_pc_kitty', T(5.5), T(6.3), 55, '🌸 [E] Dev Station (Hello Kitty)');"
);
code = code.replace(
  "check('dev_pc_spidey', T(4.5), T(15.1), 55, '🕷️ [E] Dev Station (Spider-Man)');",
  "check('dev_pc_spidey', T(5.5), T(15.1), 55, '🕷️ [E] Dev Station (Spider-Man)');"
);

// 2. Update interaction triggers in handleClick
code = code.replace(
  "if (checkClick(T(4.5), T(6.3), 60, () => this.onOpenMember?.('frontend'))) return;",
  "if (checkClick(T(5.5), T(6.3), 60, () => this.onOpenMember?.('frontend'))) return;"
);
code = code.replace(
  "if (checkClick(T(4.5), T(15.1), 60, () => this.onOpenMember?.('frontend'))) return;",
  "if (checkClick(T(5.5), T(15.1), 60, () => this.onOpenMember?.('frontend'))) return;"
);

// 3. Replace drawDevRoom with computers reversed facing left and chairs on the left side
const devRoomChairsOnLeft = `  // --- DEVELOPMENT ROOM (Themed L-Shaped Desks: Computers Reversed & Chairs on Left Side) ---
  private drawDevRoom(ctx: CanvasRenderingContext2D, S: number) {
    const isNearKitty = Math.hypot(this.state.player.x - T(5.5), this.state.player.y - T(6.3)) < 55;
    const isNearSpidey = Math.hypot(this.state.player.x - T(5.5), this.state.player.y - T(15.1)) < 55;
    ...
    const kRetX = kMainX + kMainW; // Right horizontal return credenza
    const kRetY = T(3.6);
    const kRetW = S * 2.4;
    const kRetH = S * 1.9;
    ...
    const kPcX = kMainX + 18;
    const kPcY = kMainY + S * 1.3;

    // Pastel Pink Monitor Stands (on the right of display)
    ctx.fillStyle = '#f472b6';
    ctx.fillRect(kPcX + 5, kPcY + 6, 3, 10);
    ctx.fillRect(kPcX + 5, kPcY + 30, 3, 10);
    ...
    ctx.beginPath();
    ctx.ellipse(kPcX - 6, kPcY + 23, 15, 26, 0, 0, Math.PI * 2);
    ctx.fill();

    // Pink Mechanical Keyboard in front of monitors (on Left of displays facing Left)
    ...
    for (let k = 0; k < 5; k++) {
      ctx.fillRect(kPcX - 8, kPcY + 10 + k * 5, 4, 2.5);
    }
    // Precision Mouse
    ctx.fillStyle = '#f472b6';
    ctx.fillRect(kPcX - 8, kPcY + 37, 5, 6);
    ...
    const sRetX = sMainX + sMainW; // Right horizontal return credenza
    const sRetY = T(12.6);
    const sRetW = S * 2.4;
    const sRetH = S * 1.9;
    ...
    const sPcX = sMainX + 18;
    const sPcY = sMainY + S * 1.3;

    // Red & Blue Monitor Stands (on the right of display)
    ctx.fillStyle = '#1e3a8a';
    ctx.fillRect(sPcX + 5, sPcY + 6, 3, 10);
    ctx.fillRect(sPcX + 5, sPcY + 30, 3, 10);
    ...
    ctx.beginPath();
    ctx.ellipse(sPcX - 6, sPcY + 23, 15, 26, 0, 0, Math.PI * 2);
    ctx.fill();

    // Red Mechanical Keyboard with Black Keycaps (on Left of displays facing Left)
    ...
    for (let k = 0; k < 5; k++) {
      ctx.fillRect(sPcX - 8, sPcY + 10 + k * 5, 4, 2.5);
    }
    // Web-Shooter Mouse
    ctx.fillStyle = '#1e3a8a';
    ctx.fillRect(sPcX - 8, sPcY + 37, 5, 6);
...
if (devStart !== -1 && designStart !== -1) {
  code = code.slice(0, devStart) + devRoomChairsOnLeft + "\n\n" + code.slice(designStart);
}

fs.writeFileSync('src/core/engine.ts', code, 'utf-8');
console.log('Successfully updated Dev Room: reversed computers and placed chairs on the left side!');

...
export default function BackendDevModal({ agency, manager, onClose, onRefresh }: BackendDevModalProps) {
  ...
  const handleOpenEditModal = (task: AgencyTask) => {
    ...
    setEditTaskInput({
      ...
      priority: (task.priority as any) || 'urgent',
      assignedTo: task.assignedTo || 'backend',
      estimatedHours: task.estimatedHours || 6,
      xpReward: task.xpReward || 110,
    });
  };
  ...
  const handleSaveTaskEdit = () => {
    ...
    manager.updateTask(editingTask.id, {
      ...
      estimatedHours: Number(editTaskInput.estimatedHours) || 6,
      xpReward: Number(editTaskInput.xpReward) || 110,
    });
    showToast(`✏️ Updated Backend Task: "${editTaskInput.title.trim()}"!`, '✅');
    setEditingTask(null);
    onRefresh();
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-md p-2 sm:p-4 animate-in fade-in select-none">
      <div className="relative w-full max-w-6xl h-[92vh] max-h-[920px] bg-[#080d14] border border-cyan-500/40 rounded-2xl shadow-[0_0_80px_rgba(6,182,212,0.25)] flex flex-col overflow-hidden text-slate-200 font-sans">
                          ...
                          >
                            ...
                            <div className="flex flex-wrap items-center gap-2 shrink-0">
                              ...
                              <button
                                ...
                                className="px-2.5 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 hover:border-cyan-500 cursor-pointer"
                                title="Edit task"
                              >
                                <span>✏️</span>
                                <span>Edit</span>
        ...
        {editingTask && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="bg-[#0f172a] border border-cyan-500/60 rounded-2xl w-full max-w-lg p-6 shadow-[0_0_50px_rgba(6,182,212,0.3)] space-y-4">
              <div className="flex justify-between items-center border-b border-slate-800 pb-3">
                <div>
                  <h3 className="text-lg font-bold text-white flex items-center gap-2">
                    <span className="text-cyan-400">✏️</span> Edit Task
                  </h3>
                  <p className="text-xs text-slate-400 mt-0.5">Modify backend task specifications and estimations</p>
                </div>
                <button
              ...
              <div className="space-y-3 text-xs">
                ...
                <div className="flex justify-end gap-2.5 pt-3 border-t border-slate-800">
                  ...
                  <button
                    ...
                    className="px-5 py-2 bg-gradient-to-r from-cyan-600 to-emerald-600 hover:from-cyan-500 hover:to-emerald-500 text-white font-bold rounded-xl shadow-[0_0_15px_rgba(6,182,212,0.4)] disabled:opacity-50 disabled:cursor-not-allowed transition cursor-pointer"
                  >
                    Save Changes
                  </button>
                </div>

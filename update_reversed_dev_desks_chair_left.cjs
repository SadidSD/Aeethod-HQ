...
// 3. Replace drawDevRoom with reversed L-tables wrapping chair on left
const reversedTablesChairLeft = `  // --- DEVELOPMENT ROOM (Reversed L-Tables with Chairs on Left Side) ---
  private drawDevRoom(ctx: CanvasRenderingContext2D, S: number) {
    const isNearKitty = Math.hypot(this.state.player.x - T(5.2), this.state.player.y - T(6.3)) < 55;
    const isNearSpidey = Math.hypot(this.state.player.x - T(5.2), this.state.player.y - T(15.1)) < 55;
    ...
    // Pink Mechanical Keyboard in front of monitors (facing Left)
    ...
    // Red Mechanical Keyboard with Black Keycaps (facing Left)
...
if (devStart !== -1 && designStart !== -1) {
  code = code.slice(0, devStart) + reversedTablesChairLeft + "\n\n" + code.slice(designStart);
}

fs.writeFileSync('src/core/engine.ts', code, 'utf-8');
console.log('Successfully updated Dev Room: reversed whole tables while keeping chairs on the left!');

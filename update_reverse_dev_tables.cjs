...
// 3. Replace drawDevRoom with reversed L-desks (main desk on left/center, return on left, chair on right facing left)
const reversedDevRoom = `  // --- DEVELOPMENT ROOM (Reversed Themed L-Shaped Desks: Pink Hello Kitty & Red Spider-Man) ---
  private drawDevRoom(ctx: CanvasRenderingContext2D, S: number) {
    const isNearKitty = Math.hypot(this.state.player.x - T(4.5), this.state.player.y - T(6.3)) < 55;
    const isNearSpidey = Math.hypot(this.state.player.x - T(4.5), this.state.player.y - T(15.1)) < 55;

    // =========================================================================
    // 🌸 1. TOP WORKSTATION: PINK HELLO KITTY THEMED L-DESK (Reversed)
    // =========================================================================
    const kMainX = T(3.8);
    const kMainY = T(3.6);
    ...
    const kRetX = T(1.5); // Left horizontal return credenza
    const kRetY = T(3.6);
    const kRetW = S * 2.3;
    const kRetH = S * 1.9;
    ...
    // --- Hello Kitty Dual Monitors & Battlestation (facing Right toward Chair) ---
    const kPcX = kMainX + 4;
    ...
    // Pink Mechanical Keyboard with White Keycaps (facing Right toward Chair)
    ...
    // Hello Kitty Gaming Chair on RIGHT Side (facing Left toward desk)
    this.chair(ctx, kMainX + kMainW + 14, kMainY + kMainH / 2, -Math.PI / 2, 12);


    // =========================================================================
    // 🕷️ 2. BOTTOM WORKSTATION: RED SPIDER-MAN THEMED L-DESK (Reversed)
    // =========================================================================
    const sMainX = T(3.8);
    const sMainY = T(12.6);
    ...
    const sRetX = T(1.5); // Left horizontal return credenza
    const sRetY = T(12.6);
    const sRetW = S * 2.3;
    const sRetH = S * 1.9;
    ...
    // --- Spider-Man Dual Monitors & Battlestation (facing Right toward Chair) ---
    const sPcX = sMainX + 4;
    ...
    // Red Mechanical Keyboard with Black Keycaps (facing Right toward Chair)
    ...
    // Spider-Man Racing Chair on RIGHT Side (facing Left toward desk)
    this.chair(ctx, sMainX + sMainW + 14, sMainY + sMainH / 2, -Math.PI / 2, 12);
...
if (devStart !== -1 && designStart !== -1) {
  code = code.slice(0, devStart) + reversedDevRoom + "\n\n" + code.slice(designStart);
}

fs.writeFileSync('src/core/engine.ts', code, 'utf-8');
console.log('Successfully reversed both Dev Room L-shaped tables!');
